    ...
    <div class="col-12">
      <div class="card">
        <div class="card-header">
          <h4>Data SSH (Tree: Rekening ➜ SSH)</h4>
        </div>
        ...
        <div class="card-body">
          ...
          {{-- Tabel TreeView (Level-1 = Rekening) --}}
...
$(function(){
  ...
  $('#program_filter').on('change', function(){
    ...
    if(pid){
      $.get(`{{ url('/ssh/program') }}/${pid}/kegiatan`, function(rows){
  ...
  // Cascade Kegiatan -> Sub Kegiatan
  $('#kegiatan_filter').on('change', function(){
    const kid = $(this).val();
    $('#sub_kegiatan_filter').empty().append('<option value="">-- Pilih Sub Kegiatan --</option>').prop('disabled', true).trigger('change');

    if(kid){
      $.get(`{{ url('/ssh/kegiatan') }}/${kid}/sub_kegiatan`, function(rows){
  ...
  $('#table_tree tbody').on('click', '.btn-expand', function(e){
    ...
    if(row.child.isShown()){
      ...
      // expand -> load child HTML dari server
      const params = $.param({
      ...
      $.get(`{{ url('/ssh/tree') }}/${id}/ssh?${params}`, function(html){
        row.child(html).show();
        btn.html('<i class="fas fa-chevron-down"></i>');
        tr.addClass('shown');
      }).fail(function(){
        row.child('<div class="p-3 text-danger">Gagal memuat data SSH.</div>').show();
      });
    }
  });
});
