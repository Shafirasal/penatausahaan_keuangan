...
if (kegiatanId) {
  $.get(`/realisasilpse/kegiatan/${kegiatanId}/summary`)
    .done(function(resp) {
      if (resp?.success) {
        const { total_pagu, sisa } = resp.data; // <-- total_realisasi diabaikan
  ...
  $.get(`/realisasilpse/kegiatan/${kegiatanId}/sub_kegiatan`)
    ...
    .fail(function(){ alert('Gagal memuat data sub kegiatan.'); resetSelect('#f_sub', '-- Pilih Sub Kegiatan --'); });
}
...
$('#f_sub').on('select2:select select2:clear', function(e) {
  ...
  if (e.type === 'select2:select' && $opt.length) {
    ...
    $.get(`/realisasilpse/sub_kegiatan/${subId}/rekening`)
      ...
      .fail(function(){ alert('Gagal memuat data rekening.'); resetSelect('#f_rekening', '-- Pilih Rekening --'); });
  }
});
...
$('#f_rekening').on('select2:select select2:clear', function(e) {
  ...
  if (e.type === 'select2:select' && $opt.length) {
    ...
    $.get(`/realisasilpse/rekening/${rekeningId}/ssh`)
      ...
      .fail(function(){ alert('Gagal memuat data SSH.'); resetSelect('#f_ssh', '-- Pilih SSH --'); });
  }
});
...
function resetDropdowns(dropdowns) {
  ...
  $("#form-tambah").validate({
    ...
    rules: {
      ...
      no_dokumen: { maxlength:50 },
      nilai_realisasi: {
        required:true, number:true, min:1,
        normalizer: function(value){ const v=(value||'').replace(/[^\d]/g,''); return v?String(parseInt(v,10)):''; }
      },
...
success:function(res){
  if (res?.status) {
    ...
    .then(() => {
      ...
      $('#form-tambah')[0].reset();                 // clear input text + date
      $('#i_nilai').val('');                        // pastikan kosong
      $('input[name="file"]').val('');              // clear file input

      // ── 2) kosongkan pilihan (tanpa mengubah program & kegiatan yang locked)
      //     reset dropdown sub → rekening → ssh, ringkasan & hiddennya
      resetDropdowns(['sub','rekening','ssh']);

      // ── 3) reload ringkasan pagu/sisa tingkat kegiatan (karena telah berubah)
      const kegiatanId = $('#h_kegiatan').val();
      if (kegiatanId) {
        $.get(`/realisasilpse/kegiatan/${kegiatanId}/summary`, { _: Date.now() }) // cache buster
          .done(function(resp){
            if (resp?.success) {
        ...
        // opsional: langsung muat ulang daftar sub agar siap dipilih lagi
        $.get(`/realisasilpse/kegiatan/${kegiatanId}/sub_kegiatan`, { _: Date.now() })
          .done(function(resp){
            if (resp?.success) {
              populateSelect('#f_sub', resp.data, 'id_sub_kegiatan','kode_sub_kegiatan','nama_sub_kegiatan','-- Pilih Sub Kegiatan --');
            } else {
          ...
          .fail(function(){ resetSelect('#f_sub','-- Pilih Sub Kegiatan --'); });
      }
    });
  } else {
        ...
        error:function(xhr){
          if (xhr.status === 422 && xhr.responseJSON?.errors) {
            ...
            $.each(errors, function (field, messages) {
              ...
              if ($field.length) {
                const $group = $field.hasClass('select2-hidden-accessible') ? $field.next('.select2').closest('.form-group') : $field.closest('.form-group');
              ...
              $('#error-' + field).text(messages[0]); // kalau ada span manual
            });
